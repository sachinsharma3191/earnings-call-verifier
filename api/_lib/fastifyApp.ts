...
import { findQuarterData, verifySingleClaim, calculateSummary, type Claim } from "./verify";

let appPromise: Promise<FastifyInstance> | null = null;

export function getFastifyApp(): Promise<FastifyInstance> {
  if (!appPromise) {
    appPromise = buildApp();
  }
  return appPromise;
...
async function buildApp(): Promise<FastifyInstance> {
  ...
  app.get("/api/companies/:ticker", async (req: any, reply: FastifyReply) => {
  ...
  app.get("/api/companies/:ticker/quarters", async (req: any, reply: FastifyReply) => {
    const ticker = String(req.params.ticker).toUpperCase();
  ...
  app.get("/api/companies/:ticker/metrics/:quarter", async (req: any, reply: FastifyReply) => {
    const ticker = String(req.params.ticker).toUpperCase();
  ...
  app.post("/api/verification/verify", async (req: any, reply: FastifyReply) => {
    const data = req.body ?? {};
  ...
  // Dynamic OpenAPI YAML for Claude Skill registration
  app.get("/api/openapi", async (req: any, reply: FastifyReply) => {
