...
export function extractQuarterlyData(factsData: any, quarters: number = 4): QuarterData[] {
  if (!factsData) return [];

  const quarterlyFound: Record<string, QuarterData> = {};

  try {
    const usGaap = factsData?.facts?.["us-gaap"] ?? {};

    const metrics: Record<string, keyof QuarterData> = {
      Revenues: "Revenues",
    ...
    for (const [secMetric, ourMetric] of Object.entries(metrics)) {
      ...
      for (const item of values) {
        ...
        (quarterlyFound[end] as any)[ourMetric] = item?.val;
      }
    }
...
export async function getCompanyFinancials(ticker: string, quarters: number = 4): Promise<CompanyFinancials | null> {
...
export function calculateMetrics(quarterData: QuarterData): Record<string, number> {
